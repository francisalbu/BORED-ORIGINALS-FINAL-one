...
const originals = [
  ...
  },
  {
    ...
    desc: "O Caminho. Sem pressa de chegar. Quatro noites a caminhar pelo Caminho Português, de Valença a Santiago de Compostela.",
...
function BoredOriginals({ onConquista, onActivity, onBooking, onAllExperiences, adventures: dbAdventures = [] }: { onConquista?: () => void; onActivity?: (i: number) => void; onBooking?: (i: number) => void; onAllExperiences?: () => void; adventures?: any[] }) {
  ...
  const [quickBooking, setQuickBooking] = useState<{ date: { id: string; date_range: string; status: string; spots: number; price: string }; title: string; image?: string; location?: string; activityType?: string } | null>(null);
  ...
  const openBooking = (dbIndex: number) => {
    ...
    setQuickBooking({ date: { id: d.id, date_range: d.date_range, status: d.status, spots: d.spots, price: d.price }, title: adv.title, image: adv.hero_image, location: adv.location, activityType: adv.activity_type ?? 'atividade' });
  };

  // Usar dados do Supabase se disponíveis, senão fallback para os hardcoded
  const items = dbAdventures.length > 0
    ? [
        ...dbAdventures.filter((a: any) => !a.coming_soon && !(a.interest_only ?? false)),
        ...dbAdventures.filter((a: any) => a.coming_soon || (a.interest_only ?? false)),
      ].map((a: any, i: number) => {
  ...
  return (
    <section id="originals" className="bg-brutal-black relative z-10 pt-24 pb-0">
      ...
      {quickBooking && <BookingModal date={quickBooking.date} activityTitle={quickBooking.title} activityImage={quickBooking.image} activityLocation={quickBooking.location} activityType={quickBooking.activityType} onClose={() => setQuickBooking(null)} onHome={() => setQuickBooking(null)} />}
      {/* Header */}
        ...
        <div className="flex gap-4 md:gap-5 px-4 md:px-16 pb-16" style={{ width: 'max-content' }}>
          {items.map((item, i) => (
            ...
            >
              ...
              {/* Price tag — top left */}
              {(item as any).price && !(item as any).interestOnly && !item.comingSoon && (
...
function parseDateRangeToNum(dr: string): number {
  const startM = dr.match(/^(\d+)\s+(\w{3})/);
  const yearM = dr.match(/(\d{4})/);
  if (!startM || !yearM) return 99999999;
  return parseInt(yearM[1]) * 10000 + (PT_MONTHS[startM[2]] ?? 0) * 100 + parseInt(startM[1]);
}
...
function AllExperiencesPage({ onBack, onHome, onActivity, onBooking, adventures }: { onBack: () => void; onHome?: () => void; onActivity: (i: number) => void; onBooking?: (i: number) => void; adventures: any[] }) {
  ...
  const [interestItem, setInterestItem] = useState<{ title: string; adventureId?: string; image?: string; description?: string; teaserDate?: string } | null>(null);
  const [quickBooking, setQuickBooking] = useState<{ date: { id: string; date_range: string; status: string; spots: number; price: string }; title: string; image?: string; location?: string; activityType?: string } | null>(null);
  ...
  const items = (adventures.length > 0 ? adventures.map((a: any, i: number) => ({
    ...
    desc: a.description,
    image: a.card_image,
    comingSoon: a.coming_soon || (a.interest_only ?? false),
    interestOnly: a.interest_only ?? false,
    location: a.location ?? '',
    price: a.price ?? null,
    ...
    adventureId: a.id,
    nextDate: (a.activity_dates ?? []).find((d: any) => d.status !== 'esgotado')?.date_range ?? null,
    teaserDate: a.teaser_date ?? null,
  })) : originals.map((o, i) => ({ ...o, location: '', price: null, priceNum: 0, index: i, interestOnly: false, difficulty: '', adventureId: '', nextDate: null, teaserDate: null })))
    .filter(item => {
    ...
    <div className="min-h-screen bg-brutal-black selection:bg-neon-yellow selection:text-brutal-black">
      ...
      <div className="flex flex-col md:flex-row min-h-screen">
        ...
        <aside className="hidden md:flex flex-col gap-6 w-64 flex-shrink-0 px-8 pt-12 border-r border-white/[0.06] sticky top-[61px] self-start h-[calc(100vh-61px)] overflow-y-auto">
          ...
          <div>
            <p className="text-white/50 font-body text-[10px] uppercase tracking-[0.3em] mb-3">Dificuldade</p>
            <select value={difficulty} onChange={e => setDifficulty(e.target.value)}
              className="w-full bg-white/5 border border-white/15 rounded-xl px-4 py-2.5 text-white text-xs font-body focus:outline-none focus:border-neon-yellow/40 transition-colors appearance-none cursor-pointer">
              {difficultyOptions.map(d => (
                <option key={d} value={d} className="bg-[#111]">{d === 'todas' ? 'Todas' : d}</option>
              ))}
            </select>
          </div>
            ...
            <div className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
              ...
              {difficultyOptions.map(d => (
                <button key={d} onClick={() => setDifficulty(d)}
                  className={`flex-shrink-0 px-4 py-2 rounded-full font-body text-xs font-semibold transition-all ${difficulty === d ? 'bg-neon-yellow text-brutal-black' : 'bg-white/5 text-white/50 border border-white/10'}`}>
                  {d === 'todas' ? 'Dificuldade' : d}
                </button>
              ))}
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
                {items.map((item, i) => (
                  <motion.div
                    ...
                    onClick={() => {
                      if (item.comingSoon) {
                        setInterestItem({ title: item.title, adventureId: item.adventureId, image: item.image, description: item.desc, teaserDate: item.teaserDate });
                      } else {
                  ...
                  >
                    ...
                    <div className="absolute top-3 left-3 flex gap-1.5">
                      {item.comingSoon && (
                        <span className="bg-white text-brutal-black font-body text-[8px] font-bold uppercase tracking-[0.12em] px-2 py-1 rounded-full">{item.teaserDate ?? 'Em breve'}</span>
                      )}
                    </div>
                    {/* Date badge — top right, only for available */}
                    {!item.comingSoon && item.nextDate && (
                      <div className="absolute top-3 right-3">
                        <span className="bg-neon-yellow text-brutal-black font-body text-[10px] font-bold tracking-[0.06em] px-3 py-1.5 rounded-full shadow-lg">{item.nextDate}</span>
                      </div>
                    )}
      ...
      {(item as any).price && !item.comingSoon && (
                      ...
                      <div className="flex items-center gap-2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        ...
                        >{item.comingSoon ? 'Tenho interesse' : 'Saber mais'}</button>
                      </div>
                    </div>
...
function BookingModal({ date, activityTitle, bookingType = 'standard', onClose, onHome, activityImage, activityLocation, activityType = 'atividade' }: {
  ...
  const isDormida = activityType === 'dormida';
  ...
  // Standard: total = people × basePrice (dormida = pack fixo = basePrice independente de pessoas)
  const standardTotal = isDormida ? basePrice : people * basePrice;
    ...
    >
      ...
      <div className="lg:hidden flex items-center justify-between px-6 py-4 border-b border-white/[0.08] bg-white/[0.04]">
        ...
        <div className="flex-shrink-0 ml-3 text-right">
          <p className="text-white font-body font-bold text-lg">{currentTotal}€</p>
          <p className="text-white/30 text-[10px] uppercase tracking-wider">total</p>
        </div>
      </div>

      {/* ── Main 2-col ── */}
      <div className="max-w-5xl mx-auto px-6 md:px-12 py-8 lg:py-12 flex flex-col lg:flex-row gap-16 lg:gap-24 items-start">
            ...
            <>
              {step === 1 && (
                <div className="space-y-10">
                  {/* People */}
                  <div>
                    <h2 className="text-white font-body font-bold text-3xl mb-1">{isDormida ? 'Reserva da Cabine' : 'Número de pessoas'}</h2>
                    <p className="text-white/50 text-sm mt-1">{isDormida ? 'Pack para 2 pessoas · preço fixo' : `${date.spots} lugares disponíveis nesta edição`}</p>
                  </div>
                  {isDormida ? (
                    <div className={`${rowCls} mt-2`}>
                      <span className="text-white/50 text-base">1 pack · 2 pessoas</span>
        ...
        {/* RIGHT: White card — hidden on mobile */}
          ...
          <div className="lg:sticky lg:top-24 space-y-3">
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-xl">
              ...
              <div className="divide-y divide-black/[0.07]">
                ...
                <div className="px-5 py-4">
                  <p className="text-black/40 text-xs font-bold uppercase tracking-[0.12em] mb-1">{isDormida ? 'Pack' : 'Pessoas'}</p>
                  <p className="text-black text-sm font-medium">{isDormida ? '2 pessoas' : `${people} pessoa${people > 1 ? 's' : ''}`}</p>
                </div>
                {holders[0].name && (
                  <div className="px-5 py-4">
...
function NewsletterSimple() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!isValidEmail(normalizedEmail)) {
      setError('Esse email não parece válido. Verifica o formato (ex: nome@exemplo.com).');
      return;
    }
    ...
    setError(null);
    try {
      const { error: subError, alreadySubscribed } = await subscribeNewsletter(normalizedEmail);
        ...
        >
          {submitted ? (
            ...
            <motion.div
              ...
              className="flex flex-col items-center gap-3"
            >
              <div className="w-12 h-12 rounded-full bg-white/5 border border-white/15 flex items-center justify-center text-2xl">
                😄
              </div>
              <p className="text-white font-body font-bold text-lg">Já estás registado!</p>
              <p className="text-white/35 font-body text-sm">Este email já está na nossa lista. Quando houver novidade, avisamos-te.</p>
            </motion.div>
          ) : (
...
const activityDetails = [
  ...
  ...[
    ...
    { idx: 8, title: "Caminhos de Santiago", location: "Caminho Português · Valença a Santiago", duration: "5 dias · 4 noites", difficulty: "Médio", price: "249€", image: "https://prifvutxutzcspiukzek.supabase.co/storage/v1/object/public/Originals/10.png", desc: "Quatro noites a caminhar pelo Caminho Português de Santiago, desde a fronteira em Valença até Santiago de Compostela. Dias de bosques, vilas, pontes, pedra gasta e quilómetros que se acumulam sem drama — só com o tipo de cansaço que, estranhamente, sabe bem e faz pensar.", review: "O caminho é antigo, mas a maneira de o fazer continua a ser tua.", reviewAuthor: "Participante, 2025",
      ...
      not_includes_detail: ["Transporte até Valença/Tui", "Refeições, salvo indicação contrária", "Equipamento de caminhada", "Seguro pessoal de atividade (recomendado)", "Despesas de caráter pessoal"],
      itinerary_detail: [
        { day: "Dia 1", title: "Valença / Tui até Redondela", desc: "A aventura começa na fronteira, com a saída de Valença e a entrada em Tui, onde o Caminho Português ganha logo outro tom. Esta é uma etapa longa para abrir a viagem com intenção: sai-se de Portugal, entra-se na Galiza e começa-se a sentir a cadência real do percurso. O destino da noite é Redondela, um ponto clássico de dormida e uma boa primeira paragem para alinhar corpo, mochila e cabeça." },
        { day: "Dia 2", title: "Redondela até Pontevedra", desc: "Depois de Redondela, o caminho torna-se mais fluido e mais bonito, com troços entre bosques, caminhos de terra e vistas abertas sobre a ria. A chegada a Pontevedra é particularmente interessante porque a cidade é um dos grandes marcos do Caminho Português na Galiza e funciona muito bem como dormida intermédia. É uma etapa suficientemente exigente para valer o dia, mas sem te esmagar logo a meio da viagem." },
        { day: "Dia 3", title: "Pontevedra até Caldas de Reis", desc: "Este é um dos dias mais equilibrados da rota, com uma progressão agradável entre aldeias, zonas agrícolas e troços de calma quase obstinada. A chegada a Caldas de Reis funciona bem como pausa natural antes da reta final, tanto pela logística como pela sensação de avanço claro no caminho. É o tipo de dia em que o corpo já sabe o que fazer e a cabeça começa finalmente a acompanhar." },
        { day: "Dia 4", title: "Caldas de Reis até Padrón", desc: "A quarta etapa leva-te até Padrón, um dos nomes mais importantes antes da entrada final em Santiago. É um dia que pede consistência, não heroísmo, e fecha a experiência com a certeza de que o final já está próximo, mas ainda merece ser caminhado com atenção. Dormir em Padrón dá equilíbrio à estrutura de 4 noites e prepara a chegada final no dia seguinte." },
        { day: "Dia 5", title: "Padrón até Santiago de Compostela", desc: "A última etapa é a mais simbólica de todas. Saindo de Padrón, o caminho leva-te até Santiago de Compostela, cruzando os últimos quilómetros com uma mistura de expectativa, cansaço e foco silencioso. A entrada na cidade fecha a peregrinação de forma clássica e limpa, sem excesso de drama, mas com a carga emocional certa." },
      ],
      packing_detail: ["Botas ou ténis de caminhada confortáveis", "Mochila de caminhada leve", "Roupa técnica respirável", "Corta-vento ou impermeável leve", "Meias de caminhada extra", "Garrafa de água reutilizável", "Chapéu ou boné", "Protetor solar", "Bastões, se usares", "Lanterna frontal", "Artigos de higiene pessoal", "Muda de roupa confortável"],
      faqs_detail: [
        { q: "Qual o nível físico necessário?", a: "Médio. A rota é acessível a quem esteja habituado a caminhar vários dias seguidos, desde que aceite um ritmo constante e sem pressa." },
        { q: "É preciso experiência em caminhadas longas?", a: "Não é obrigatório, mas ajuda sentir-te confortável com dias sucessivos de 15 a 30 km, dependendo da etapa." },
        { q: "Quantas noites inclui?", a: "A experiência está desenhada para 4 noites, com dormida em pontos-chave ao longo do Caminho." },
        { q: "As refeições estão incluídas?", a: "Não. O alojamento está incluído, mas as refeições ficam fora do pacote para dar mais flexibilidade ao viajante. Em algumas estadias, um ligeiro pequeno-almoço pode estar incluído." },
      ],
      curiosities_detail: [
        { title: "O Caminho Português é uma rota com memória forte.", text: "A partir de Valença, o percurso entra na Galiza por Tui e segue por um dos troços mais clássicos e reconhecidos do Caminho Português, com pequenas vilas, bosques e cidades históricas ao longo de toda a travessia." },
        { title: "As etapas ficam mais bonitas quando deixam de querer impressionar.", text: "Entre Tui, O Porriño, Redondela, Pontevedra, Caldas de Reis e Padrón, a rota mistura zonas urbanas, caminhos florestais e troços rurais que tornam a caminhada variada sem a tornar extrema." },
        { title: "Chegar a Santiago é menos sobre destino e mais sobre processo.", text: "A cidade funciona como ponto final simbólico, mas o que fica é a forma como atravessaste os dias até lá chegar: a repetição, o ritmo, as pausas e a estranha clareza que só aparece a andar." },
      ],
      cancellations_detail: "Cancelamentos devem ser enviados por e-mail para bookings@boredtourist.com. O cancelamento é gratuito até 10 dias antes da partida. Se o cancelamento ocorrer após essa data e até 48 horas antes do início da actividade, o cancelamento tem um custo de 50%. A menos de 48 horas, não há direito a reembolso. Em caso de cancelamento pela Bored. ou pelo prestador por motivos de segurança, meteorologia ou força maior, oferecemos nova data ou reembolso total.",
    },
    ...
    title: a.title, tagline: (a as any).tagline ?? a.desc.split('.')[0] + '.', location: a.location, duration: a.duration,
    ...
    includes: (a as any).includes_detail ?? [
    ...
    notIncludes: (a as any).not_includes_detail ?? ["Transporte até ao ponto de partida", "Refeições não indicadas no programa", "Despesas pessoais"],
    itinerary: (a as any).itinerary_detail ?? [
    ...
    packingList: (a as any).packing_detail ?? ["Calçado adequado à atividade", "Mochila de dia", "Protetor solar", "Garrafa de água reutilizável", "Roupa em camadas", "Documento de identidade"],
    faqs: (a as any).faqs_detail ?? [
    ...
    cancellations: (a as any).cancellations_detail ?? "Cancelamento gratuito até 30 dias antes. Entre 30-15 dias, 50% de reembolso. Menos de 15 dias, sem reembolso.",
  })),
];
...
const activityDates: Record<number, { range: string; status: 'disponivel' | 'apreencher' | 'esgotado'; spots: number; price: string }[]> = {
  ...
  8: [{ range: '1 Set — 5 Set 2026', status: 'disponivel', spots: 8, price: '249€' }, { range: '6 Out — 10 Out 2026', status: 'disponivel', spots: 8, price: '249€' }],
...
function ActivityPage({ activityIndex, onBack, autoBook = false, allAdventures = [] }: { activityIndex: number; onBack: () => void; autoBook?: boolean; allAdventures?: any[] }) {
  ...
  useEffect(() => {
    ...
    getAdventureByIndex(activityIndex).then(adv => {
      if (adv) {
        // inject all adventures for "outras aventuras" section
        setDbAdv({ ...adv, _allAdventures: allAdventures });
        if (autoBook) {
  ...
  const data = dbAdv ? {
    ...
    curiositiesImages: dbAdv.curiosities_images ?? [],
    ...
    <div className="min-h-screen bg-brutal-black text-white font-body overflow-x-hidden">
      ...
      <div ref={tabsRef} className="px-4 md:px-20 py-16 md:py-28 border-b border-white/6">
        ...
        <div className="flex flex-col gap-3 max-w-4xl mx-auto">
          {bookingDate && <BookingModal date={bookingDate} activityTitle={data.title} activityImage={data.heroImage} activityLocation={data.location} activityType={dbAdv?.activity_type} onClose={() => setBookingDate(null)} onHome={onBack} />}
          ...
          {dates.map((d: any, i: number) => {
            const isDormidaDate = dbAdv?.activity_type === 'dormida';
            const isAvailable = d.status === 'disponivel' || d.status === 'apreencher';
            const isFilling = false;
            const isSoldOut = d.status === 'esgotado';
            const spotsNum = d.spots ?? 0;
            const spotsLabel = isDormidaDate
              ? 'Disponível'
              : isAvailable
              ? `${spotsNum} lugares disponíveis`
              : '';
      ...
      <div className="mt-4">
      {(() => {
        const curiositiesImages: string[] = (data as any).curiositiesImages ?? [];
        const curiosities: { title: string; text: string; image: string }[] = dbAdv?.curiosities?.length
          ? dbAdv.curiosities.map((c: { title: string; text: string; image?: string }, i: number) => ({
              ...c,
              image: curiositiesImages[i] ?? c.image ?? '',
            }))
          : [];
      ...
      <div className="px-10 md:px-20 py-20 border-t border-white/6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {(dbAdv?._allAdventures ?? []).filter((o: any) => o.title !== data.title).slice(0, 4).map((o: any, i: number) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              ...
              onClick={() => { navigate(`/actividade/${slugify(o.title)}`); window.scrollTo({ top: 0, behavior: 'instant' }); }}
              className="group relative rounded-3xl overflow-hidden cursor-pointer"
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                ...
                <p className="text-white/40 font-body text-[11px] leading-relaxed line-clamp-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">{o.tagline ?? o.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
