...
/**
 * Derive the lightweight mobile version of a hero video from its URL, following
 * the convention  X.mp4 → X-mobile.mp4  (generated by scripts/make_mobile_videos.sh
 * and stored next to the original in Supabase). This lets phones play a ~2-4MB
 * clip instead of the 20-56MB original, while desktop keeps the full-quality file.
 * Returns undefined if there's no .mp4 to convert.
 */
function mobileVideo(url: string | null | undefined): string | undefined {
  if (!url) return undefined;
  if (/-mobile\.mp4$/i.test(url)) return url;
  return url.replace(/\.mp4(\?.*)?$/i, '-mobile.mp4$1');
}
